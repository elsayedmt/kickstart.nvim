...
end, '[R]ust [R]unnables (run/debug)')

map('n', '<leader>rt', function()
  vim.cmd.RustLsp 'testables'
end, '[R]ust [T]estables (run/debug tests)')

map('n', '<leader>rd', function()
  vim.cmd.RustLsp { 'runnables', bang = true }
end, '[R]ust [D]ebug last runnable')
...
end, 'Open external [D]ocs (docs.rs)')

map('n', '<leader>rx', function()
  vim.cmd.RustLsp 'explainError'
end, '[R]ust e[X]plain error')
...
end, '[R]ust crate [G]raph')

map('n', '<leader>rs', function()
  vim.cmd.RustLsp 'syntaxTree'
end, '[R]ust [S]yntax tree')
...
map('n', '<leader>rmu', function()
  vim.cmd.RustLsp { 'moveItem', 'up' }
end, '[R]ust [M]ove item [U]p')

map('n', '<leader>rmd', function()
  vim.cmd.RustLsp { 'moveItem', 'down' }
end, '[R]ust [M]ove item [D]own')

-- Rebuild proc macros
map('n', '<leader>rm', function()
  vim.cmd.RustLsp 'rebuildProcMacros'
end, '[R]ust rebuild proc [M]acros')
